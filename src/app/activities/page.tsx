...
export default async function ActivitiesPage() {
  ...
  return (
    <div>
      ...
      <section style={{ contentVisibility: "auto" } as React.CSSProperties} className="mx-auto max-w-7xl px-6 py-16">
        {activities.length === 0 ? (
          <CSSFadeIn>
          ...
          <CSSStagger stagger={80} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {activities.map((item) => (
              <Link key={item.id} href={`/activities/${item.slug}`} className="group block overflow-hidden rounded-2xl border border-[#dce3ed] bg-white transition-all hover:shadow-xl hover:shadow-[#082b59]/5">
