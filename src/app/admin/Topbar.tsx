  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200/80 bg-white/80 backdrop-blur-xl px-4 sm:px-6">
      ...
      <div className="ml-auto flex items-center gap-2">
        {/* Notification bell */}
        <button className="relative flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
          <Bell className="h-[18px] w-[18px]" />
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-[#1767b1]" />
        </button>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <div className="hidden text-left sm:block">
              ...
              <p className={`text-[10px] font-bold uppercase ${roleBadge[profile.role] || "text-slate-500"}`}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-slate-200 bg-white py-1.5 shadow-xl">
              <div className="px-4 py-2.5 border-b border-slate-100">
                ...
                <span className={`inline-block rounded-md px-1.5 py-0.5 text-[10px] font-bold uppercase mt-1 ${roleBadge[profile.role] || "bg-slate-100 text-slate-500"}`}>
                  {roleLabel[profile.role] || profile.role}
                </span>
              </div>
              <button
